...
public class CubeInteraction42 : MonoBehaviour
{
    public Slider slider;  // 关联 Slider 组件
    public string targetCubeTag = "Cube4";  // 目标 Cube 的标识（tag）
    ...
    void Update()
    {
        ...
        if (IsPlayerCollidingWithCube1() && sliderValue > 1f / 3f && sliderValue < 2f / 3f && animator.GetCurrentAnimatorStateInfo(0).IsName("walk2"))
        {
            // 启动协程
